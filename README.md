...
URL calculations accept optional per-load `:service_opts`, merged into the configured storage service options for URL generation only. This is useful for request-specific options like S3 expiry settings or S3-compatible endpoint overrides:

```elixir
post =
  Ash.load!(post,
    cover_image_url: %{service_opts: [presigned: true, expires_in: 60]}
  )

post.cover_image_url
#=> "https://s3.storage.example/my-bucket/a81bf21e2442...?X-Amz-Expires=60"
```
