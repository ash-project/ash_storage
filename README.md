...
- ~~**Per-variant oban jobs**~~ ✅ — Each `generate: :oban` variant now gets its own Oban job lifecycle, enabling parallel generation and independent retries. Attach enqueues one `:run_pending_variant` job per variant; a cron-driven `:run_pending_variants` trigger re-fans-out any blob still flagged `pending_variants == true` for recovery. Per-variant completion uses a `jsonb_set`-based atomic update on Postgres so concurrent jobs don't clobber each other.
